...
void ChatService::login(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (user.getId() != -1 && user.getPwd() == pwd)
    {
        if (user.getState() == "online")
        {
            ...
            response["errno"] = 2;
            response["errmsg"] = "this accout is using, intput another!";
        ...
        else
        {
            // 登陆成功，记录用户连接信息
            {
                lock_guard<mutex> lock(_connMutex); // 大括号作用域内锁
            ...
            if (!groupVec.empty())
            {
                ...
                for (Group &group : groupVec)
                {
                    ...
                    vec1.push_back(grpjs.dump());
                }
                response["groups"] = vec1;
            }

            conn->send(response.dump()); // send函数将数据从用户空间的缓冲区发送到内核空间的网络协议栈，最终通过网络传输到对端
        }
    }
    else
    {
...
void ChatService::oneChat(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    {
        lock_guard<mutex> lock(_connMutex); // 锁的范围要尽可能小
